...
## Import scRNAseq data and generate cellphonedb inputs
...
```{r}
plot.data=plot.data[order(plot.data$clusters, decreasing = F),]
plot.data$clusters=as.character(plot.data$clusters)

res=data.frame()
for (i in 1:nrow(plot.data)){
  res1=plot.data[i,c(1:4,seq(5,9,2),11)]
  res2=plot.data[i,c(1:4,seq(6,10,2),11)]
